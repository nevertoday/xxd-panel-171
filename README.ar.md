...
# XXD Panel 171｜هيكل رأسي ومشاهد متناثرة دقيقة
...
فراغ أبيض دافئ بنسبة 75–85%، مع خطوط رأسية رفيعة منخفضة التباين وزخارف سردية دقيقة متباعدة كالجزر.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 171 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-171`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-171 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-171-prompt.en.md) أو [بالصينية](references/xxd-panel-171-prompt.zh-CN.md).
