...
生成時のPNGを維持し、拡大縮小・切り抜き・境界補正はしていません。横は1672×941（約16:9）、縦は1086×1448（3:4）です。横の境界は中央付近、縦の元画像領域は約46.8–50.0%で、すべて厳密な50:50ではありません。
...
75–85%の暖白余白に、細く低コントラストの縦線と孤島のような微小モチーフを疎らに配置。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 171 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-171` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-171 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-171-prompt.en.md)／[中国語](references/xxd-panel-171-prompt.zh-CN.md)を参照してください。
